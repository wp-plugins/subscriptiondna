...
var frmValidate = function(frm){	
	if(frm.cc_type.value=="")
	{
		//alert('Credit card type can not be left blank.');
                var span = SubscriptionDNA_GetElement('cc_type_lbl_error');
                var errMsg = "Credit card type can not be left blank" ;
                showErrorMsg(span,errMsg);
		frm.cc_type.focus();
		return false;
	}
	if(frm.cc_exp_month.value=="" || frm.cc_exp_year.value=="")
	{
		//alert('Credit card type can not be left blank.');
                var span = SubscriptionDNA_GetElement('cc_exp_lbl_error');
                var errMsg = "Credit card expiry info is required" ;
                showErrorMsg(span,errMsg);
	...
	if(frm.cc_name.value=='')
	{
		//alert('Card holder name can not be left blank.');
                var span = SubscriptionDNA_GetElement('cc_name_lbl_error');
                var errMsg = "Card holder name can not be left blank" ;
                showErrorMsg(span,errMsg);
		frm.cc_name.focus();
		return false;
	}
	if(frm.cc_number.value=='')
	{
		//alert('Credit card number can not be left blank.');
                var span = SubscriptionDNA_GetElement('cc_number_lbl_error');
                var errMsg = "Credit card number can not be left blank" ;
                showErrorMsg(span,errMsg);
		frm.cc_number.focus();
		return false;
	}
	if(!ValidateCC( frm.cc_number.value ) ){
		//alert("Invalid Credit Card Number");
                var span = SubscriptionDNA_GetElement('cc_number_lbl_error');
                var errMsg = "Invalid Credit Card Number" ;
                showErrorMsg(span,errMsg);
		frm.cc_number.focus();
		return false;
	}
...
function dnaPaymentMethodChanged(mthd)
{
	...
	else if(mthd==2)
	{
            ...
            document.getElementById('trCCInfo5').style.display="";
            document.getElementById('existingCCInfo').style.display="none";
	}
        else{
